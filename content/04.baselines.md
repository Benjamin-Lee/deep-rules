...
While performance baselines available from existing literature also serve as a helpful guidance and should be taken into account, an implementation of a simple model (for example, linear or logistic regression) using the same software framework planned for deep learning is additionally helpful for assessing the correctness of data processing and performance evaluation pipelines.
...
Such hybrid models that combine deep learning and simpler models can improve generalization performance, model interpretability, and confidence estimation [@arxiv:1803.04765; @arxiv:1805.11783].
In addition, be sure to tune and compare current state-of-the-art tools (for example, bioinformatics pipelines or image analysis workflows), regardless of whether they use machine learning, in order to gauge the relative effectiveness of your baseline and deep learning models.
...
It is worth noting that conventional off-the-shelf machine learning algorithms (for example, support vector machines and random forests) are also likely to benefit from hyperparameter tuning.
It can be tempting to train baseline models with these conventional methods using default settings, which may provide acceptable but not stellar performance, but then tune the settings for deep learning algorithms to further optimize performance.
...
Those wishing to compare different learning algorithms should tune the settings of both traditional and deep learning-based methods to optimize performance before making claims about relative performance differences.
The performance comparison among deep learning models and many other machine learning approaches is informative only when the models are similarly well tuned.

To sum this tip up, create and fully tune several traditional models and standard pipelines before implementing a deep learning model.
