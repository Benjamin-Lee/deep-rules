...
Users who apply a VAE architecture to a new dataset with more input features, even without changing any hyperparameters, alter the relative weights of the components of the loss function. **{BDL: I'm not entirely sure this sentence makes sense, nor how to make this second person.}**
...
This effect also has implications if you seek to use existing deep learning methods because performance estimates from deep neural networks are often provided after tuning.
The implication of this effect is that attaining performance numbers that match those reported in publications is likely to require a relatively large input of human and computation time for hyperparameter optimization.
