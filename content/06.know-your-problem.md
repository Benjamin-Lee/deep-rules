...
It is important to note that often molecular or imaging datasets require appropriate clinical or demographic data to support robust analyses; this must always be balanced with the need to protect patient privacy (see [Tip 10](#privacy)).
...
One tool in the practicioner's toolkit for dealing with datasets that are not thoroughly understood is data simulation.
Simulating data under reasonable assumptions and assessing the extent to which the model is capable of capturing or mitigating issues with certain features of the data is a valuable technique for identifying model architectures and hyperparameters that are reasonable under those assumptions.
