...
A publication or resource might purportedly offer data that seem to be a good fit to test your hypothesis, but the act of obtaining the data can reveal numerous problems such as that the data are unstructured when they are supposed to be structured, crucial metadata such as sample stratification are missing, or the usable sample size is different than what is reported.
...
Once the data are obtained, it is easy to begin analyzing data without a good understanding of the study design, namely why the data was collected and how.
